using Discord;
using Discord.WebSocket;
using System;
using System.Collections;
using System.Collections.Generic;
using System.Diagnostics;
using System.Reflection;
using System.Text;
using System.Threading.Tasks;

namespace JamyShell.Modules
{
    public class Logs
    {
        public const int Type_Debug = -1;
        public const int Type_Info = 0;
        public const int Type_Notice = 1;
        public const int Type_Warn = 2;
        public const int Type_Alert = 3;

        public Program Instance = Program.GetInstance();

        public Task DisplayLogs(LogMessage message)
        {
            switch (message.Severity)
            {
                case LogSeverity.Debug:
                    Debug(message.Message);
                    break;
                case LogSeverity.Info:
                    Info(message.Message);
                    break;
                case LogSeverity.Critical:
                    Alert(message.Message);
                    break;
                default:
                    Console.WriteLine(message);
                    break;
            }
            return Task.CompletedTask;
        }
        public Logs Alert(string message)
        {
            Instance.GetLogChannel().SendMessageAsync(embed: EmbedLogs(Type_Alert, message));
            Console.ForegroundColor = ConsoleColor.Red;
            WriteLogs(message);
            return this;
        }

        public Logs Debug(Object message)
        {
            Console.ForegroundColor = ConsoleColor.Cyan;
            WriteLogs(message);
            return this;
        }

        public Logs Info(string message)
        {
            Console.ForegroundColor = ConsoleColor.Green;
            WriteLogs("[Info] "+message);
            return this;
        }
        public Logs Warn(string message)
        {
            Console.ForegroundColor = ConsoleColor.Yellow;
            WriteLogs(message);
            return this;
        }
        public Logs Notice(string message)
        {
            Console.ForegroundColor = ConsoleColor.Magenta;
            WriteLogs(message);
            return this;
        }

        private void WriteLogs(Object message)
        {
            Console.WriteLine(GetTime() + message);
            Console.ResetColor();
        }
        private string GetTime()
        {
            return DateTime.UtcNow.ToString("[HH:mm:ss] ");
        }

        private Embed EmbedLogs(int type, string message)
        {
            var color = Color.Blue;
            string title = "LOGS";
            switch (type)
            {
                case Type_Alert:
                    color = Color.DarkRed;
                    title = "ALERT";
                    break;
            }
            return new EmbedBuilder()
                .WithColor(color)
                .WithTitle(title)
                .WithUrl("https://github.com/Palente")
                .WithDescription("**Message**:\n> " + message)
                .WithCurrentTimestamp()
                .WithFooter("@Secure - Heberg x Palente")
                .Build();
        }

        public string Var_Dump(object obj, int recursion = 0)
        {
            StringBuilder result = new StringBuilder();

            // Protect the method against endless recursion
            if (recursion < 5)
            {
                // Determine object type
                Type t = obj.GetType();

                // Get array with properties for this object
                PropertyInfo[] properties = t.GetProperties();

                foreach (PropertyInfo property in properties)
                {
                    try
                    {
                        // Get the property value
                        object value = property.GetValue(obj, null);
                        // Create indenting string to put in front of properties of a deeper level 
                        // We'll need this when we display the property name and value
                        string indent = String.Empty;
                        string spaces = "|   ";
                        string trail = "|...";
                        if (recursion > 0)
                        {
                            indent = new StringBuilder(trail).Insert(0, spaces, recursion - 1).ToString();
                        }
                        if (value != null)
                        {
                            // If the value is a string, add quotation marks
                            string displayValue = value.ToString();
                            if (value is string) displayValue = String.Concat('"', displayValue, '"');
                            // Add property name and value to return string
                            result.AppendFormat("{0}{1} = {2}\n", indent, property.Name, displayValue);
                            try
                            {
                                if (!(value is ICollection))
                                {
                                    // Call var_dump() again to list child properties
                                    // This throws an exception if the current property value 
                                    // is of an unsupported type (eg. it has not properties)
                                    result.Append(Var_Dump(value, recursion + 1));
                                }
                                else
                                {
                                    // 2009-07-29: added support for collections
                                    // The value is a collection (eg. it's an arraylist or generic list)
                                    // so loop through its elements and dump their properties
                                    int elementCount = 0;
                                    foreach (object element in ((ICollection)value))
                                    {
                                        string elementName = String.Format("{0}[{1}]", property.Name, elementCount);
                                        indent = new StringBuilder(trail).Insert(0, spaces, recursion).ToString();

                                        // Display the collection element name and type
                                        result.AppendFormat("{0}{1} = {2}\n", indent, elementName, element.ToString());

                                        // Display the child properties
                                        result.Append(Var_Dump(element, recursion + 2));
                                        elementCount++;
                                    }

                                    result.Append(Var_Dump(value, recursion + 1));
                                }
                            }
                            catch { }
                        }
                        else
                        {
                            // Add empty (null) property to return string
                            result.AppendFormat("{0}{1} = {2}\n", indent, property.Name, "null");
                        }
                    }
                    catch
                    {
                        // Some properties will throw an exception on property.GetValue() 
                        // I don't know exactly why this happens, so for now i will ignore them...
                    }
                }
            }
            return result.ToString();
        }
    }
}
